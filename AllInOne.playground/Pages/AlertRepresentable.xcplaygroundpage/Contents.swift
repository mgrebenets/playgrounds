...
public extension AlertRepresentableError where Self: CustomErrorConvertible {
    ...
    var alertTitle: String {
        guard let title = message.components(separatedBy: messageSeparator).first else {
    ...
    public var alertMessage: String {
        guard let text = message.components(separatedBy: messageSeparator).last else {
            return message
        }
...
fileprivate extension String {
    ...
    func hasExternalLink() -> Bool {
        return contains(MessageKeys.netbankWebsiteLink)
    }

    /// Check if message contains CBA phone number
    func hasPhoneNumber() -> Bool {
        return contains(MessageKeys.customerServiceMessage) || contains(MessageKeys.customerServiceMessage)
    }
}
